...
func TestSerializeRecord(test *testing.T) {
	...
	serialize, err := serializeRecord(testServer, "atag", record, "lm", defaultId)
...
func (p *TestPlugin) Flush(*LmClient) int                                       { return output.FLB_OK }

func (p *TestPlugin) Send(log []byte, client *LmClient) int {
...
func TestPluginFlusher(test *testing.T) {
	...
	require.Equal(test, j["output_id"], "lm_output_1")
